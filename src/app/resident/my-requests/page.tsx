...
const statusConfig: Record<string, { color: string; icon: string; stepIndex: number }> = {
  pending: { color: '#ff8439', icon: 'hourglass_top', stepIndex: 0 },
  survey: { color: '#699cff', icon: 'location_searching', stepIndex: 1 },
  approved: { color: '#69f6b8', icon: 'verified', stepIndex: 2 },
  scheduled: { color: '#699cff', icon: 'calendar_month', stepIndex: 3 },
  planted: { color: '#69f6b8', icon: 'park', stepIndex: 4 },
  monitoring: { color: '#69f6b8', icon: 'monitor_heart', stepIndex: 5 },
};
  ...
  return (
    <div className="min-h-screen bg-[#060e20] grid-pattern relative overflow-hidden">
      ...
      <header className="fixed top-0 w-full z-50 glass-overlay border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4 md:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="hidden sm:block text-xs text-[#a3aac4]">My Requests</span>
          </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full glass-overlay border-t border-white/5 flex justify-around items-center h-16 z-50">
        <Link href="/resident" className="flex flex-col items-center gap-0.5 text-[#6d758c]">
        ...
        <Link href="/resident/my-requests" className="flex flex-col items-center gap-0.5 text-[#69f6b8]">
      ...
      <main className="pt-16 pb-20 md:pb-8 relative z-10">
        <div className="max-w-5xl mx-auto px-4 md:px-8 py-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8 animate-reveal-up">
            ...
            <Link href="/resident/request-tree" className="px-6 py-3 bg-gradient-to-r from-[#69f6b8] to-[#06b77f] text-[#002919] font-bold rounded-xl shadow-lg shadow-[#69f6b8]/20 transition-all flex items-center gap-2 btn-shine">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8 animate-reveal-up" style={{ animationDelay: '0.1s' }}>
            {[
              ...
              { label: 'Trees Planted', value: plantedCount, icon: 'park', color: '#69f6b8' },
              { label: 'CO₂ Offset', value: '480 lbs', icon: 'co2', color: '#69f6b8' },
              { label: 'Cooling Impact', value: '1.2°F', icon: 'thermostat', color: '#ff8439' },
            ].map((stat) => (
              ...
              {requestLifecycle.map((stage, idx) => (
                <div key={stage.step} className="flex items-center gap-1 min-w-0">
                  <div className="flex flex-col items-center min-w-[64px]">
                    <div className="h-9 w-9 rounded-xl bg-[#060e20]/60 border border-white/5 flex items-center justify-center">
                      <span className="material-symbols-outlined text-sm text-[#6d758c]">{stage.icon}</span>
                    </div>
                    <span className="text-[10px] text-[#6d758c] mt-1.5 text-center font-semibold">{stage.step}</span>
                  </div>
                  {idx < requestLifecycle.length - 1 && (
                    <div className="h-px bg-white/5 flex-1 min-w-[12px]" />
                  )}
                </div>
              ))}
            </div>
                  ...
                  <div className="p-5 md:p-6">
                    <div className="flex flex-col md:flex-row items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-4 mt-3 text-xs text-[#6d758c]">
                      ...
                      <div className="flex items-center gap-2 overflow-x-auto pb-1">
                        {requestLifecycle.slice(0, config.stepIndex + 1).map((stage, idx) => (
                          <div key={stage.step} className="flex items-center gap-1">
                            ...
                            <span className="text-[10px] text-[#6d758c] whitespace-nowrap">{stage.step}</span>
