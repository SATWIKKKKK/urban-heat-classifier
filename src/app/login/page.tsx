      ...
      {loginToast && (
        <div className="fixed top-6 right-6 z-[100] animate-reveal-up">
          <div className="glass-card rounded-2xl px-6 py-4 glow-primary flex items-center gap-3">
            <span className="material-symbols-outlined text-[var(--green-400)] text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span>
            <div>
              <p className="text-white font-bold text-sm">Signed in as {loginToast.role.replace(/_/g, ' ')}</p>
              <p className="text-[var(--text-secondary)] text-xs">{loginToast.name}</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="glass-card rounded-3xl p-8 md:p-10 glow-primary relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-[var(--text-secondary)] mt-1">Sign in to your city dashboard</p>
            </div>
            ...
            <div className="space-y-2">
              <button
                ...
                className="w-full flex items-center justify-center gap-3 px-4 py-3.5 glass-card hover:border-white/15 rounded-xl transition-all text-white font-medium group disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {googleLoading ? (
                ...
                <span className="group-hover:text-[var(--green-400)] transition-colors">
                  {googleLoading ? "Connecting to Google..." : "Continue with Google"}
                </span>
              </button>
              {googleError && (
                <p className="text-center text-[12px] text-[#ef4444] animate-reveal-up">
                  {googleError}
                </p>
              )}
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm">
                <label className="flex items-center gap-2 cursor-pointer text-[var(--text-secondary)] group">
                  <input type="checkbox" className="rounded border-white/20 bg-[var(--bg-elevated)] accent-[var(--green-400)]" />
                  <span className="text-xs group-hover:text-white transition-colors">Remember me</span>
                </label>
                <a href="#" className="text-xs text-[var(--green-400)] hover:underline underline-offset-4">
                  Forgot password?
                </a>
              </div>
            ...
            <p className="text-center text-sm text-[var(--text-secondary)] mt-6">
              ...
              <Link href="/register" className="text-[var(--green-400)] hover:underline font-semibold underline-offset-4">
                Register
              </Link>
            </p>
          </div>
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[var(--bg-base)] grid-pattern" />}>
      <LoginPageContent />
    </Suspense>
  );
}
